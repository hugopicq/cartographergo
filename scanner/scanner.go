...
//Find open sockets
func Scan(addresses []string, ports []uint16, timeout uint) []Socket {
	opensockets := []Socket{}
...
func GetComputersLDAP(domaincontroller string, user string, password string, domain string) []Computer {
	ldapConnection, err := ldap.DialURL("ldap://" + domaincontroller + ":389")
	if err != nil {
		log.Fatal("Error while dialing the remote server: " + err.Error())
	}
	defer ldapConnection.Close()

	err = ldapConnection.Bind(user+"@"+domain, password)
	if err != nil {
		log.Fatal("Error while authenticating to the DC: " + err.Error())
	}

	//Then we get the computers
	baseSuffix := ""
	...
	result, err := ldapConnection.SearchWithPaging(searchReq, 256)
	if err != nil {
		log.Fatal("Error while retrieving results: " + err.Error())
	}

	computers := make([]Computer, len(result.Entries))
	for c, computer := range result.Entries {
...
func (c Computer) ToCSVLine() []string {
	line := []string{c.Name, c.IP, c.OperatingSystem}
